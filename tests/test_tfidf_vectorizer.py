...
from glassboxml.feature_extraction import TfidfVectorizer
...
def test_engine_output_types(sample_corpus):
    ...
    assert isinstance(sparse_matrix, sp.csr_matrix), (
        "Sparse mode failed to return a SciPy CSR matrix."
    )
    ...
    assert isinstance(dense_matrix, np.ndarray), (
        "Dense mode failed to return a pure NumPy array."
    )
...
def test_vocabulary_consistency(sample_corpus):
    ...
    assert sparse_vec.vocabulary_ == dense_vec.vocabulary_, (
        "Engines built different dictionaries!"
    )
    assert sparse_vec.n_features_ == dense_vec.n_features_, (
...
def test_dual_engine_mathematical_parity(sample_corpus):
    ...
    """
    sparse_matrix = (
        TfidfVectorizer(mode="sparse").fit_transform(sample_corpus).toarray()
    )
    ...
    assert np.allclose(sparse_matrix, dense_matrix), (
        "CRITICAL: Mathematical parity broken between engines!"
    )
