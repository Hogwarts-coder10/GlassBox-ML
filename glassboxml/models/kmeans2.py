...
class KMeansClustering(GlassBoxModel):
    ...
    def fit(self, X, y=None):
        ...
        # Trackers for the ultimate winner across all 10 runs
        best_inertia = float('inf')
        best_centroids = None
        best_clusters = None
        best_labels = None
        
        # We don't want to spam the terminal 10 times, so we just print once
        # print(f"K-Means v2: Running {self.n_init} initializations to find the global minimum...")
        
        for init_run in range(self.n_init):
            # --- 1. Random Drop (New drop for every run) ---
            ...
            for i in range(self.max_iters):
                ...
                # If they stop moving, break out of this specific initialization early
                if np.all(old_centroids == current_centroids):
                    break
                    
            # --- 3. Calculate Inertia for THIS specific run ---
            current_inertia = 0.0
            for cluster_idx, cluster_points in enumerate(current_clusters):
                if len(cluster_points) > 0:
            ...
            # --- 4. The v2 Check: Is this the best run yet? ---
        ...
        # --- 5. Lock in the absolute best result to the class variables ---
